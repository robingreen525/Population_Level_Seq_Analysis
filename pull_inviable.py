# the purpose of this script is to extract the inviable  mutations (so essential genes as annotated by SGD) in the 'master.csv'
#file that I have generated so I restrict my analysis only to SNPs that could potentially 
# result in invability. I will do this to see if I see genes putative deleterious mutations in essential genes at the same or similar frequencies in Aaron's populations
# compared to my previously found auxotrophy mutations to get a sense of the amount of background noise in my data

#usage:
#python Population_Level_Seq_Analysis/pull_inviable.py 
# -inviablefile /fh/fast/shou_w/NextGenSeq/reference/inviable_annotations.txt
...
parser.add_argument('-inviablefile',type=argparse.FileType('r'))
...
ll=args.inviablefile.readlines()
for line in ll:
	line=line.split('\t')
...
newfile=path+'/inviable_snps.csv'
a=open(newfile,'w')
