...
#usage
 ...
 #-inviablefile /fh/fast/shou_w/NextGenSeq/PopSeq/MDunham/Miller_SulfateGlucosePhosphate_2015/auxotrophy_snps.csv
...
ll=args.inviablefile.readlines() # read auxotrophy file, parse through for each SNP

filepath=path+'/'+'inviable_snps_SIFT.csv'
p=open(filepath,'w')

print('Merging SIFT info the inviablefile....')
for line in ll:
	...
	if(line[0]!='sample'):
		pop=line[0]
		pop1=line[0] # copy of population variable for later
		print pop
		pop=path+pop
		print pop
		os.chdir(pop)
		...
		chr=line[1]
		pos=line[2]
		file=pop1+'.freebayes_SIFTpredictions.vcf'
		g=open(file,'r')
		gg= g.readlines
		for gline in g:
			gline=gline.rstrip('\n')
